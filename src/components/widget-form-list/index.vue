<template>
	<draggable v-model="pageData[list]" :group="{name:'widget'}" ghostClass="ghost" :swapThreshold="0.5" :animation="100"
	 @add="handleWidgetAdd" class="widget-form-list" :class="{'widget-empty': pageData[list].length === 0}">
		<template v-for="(item, index) in pageData[list]">
			<div class="widget-view" :key="item.key" :class="{active: selectWg.key === item.key}" @click="handleSelectWidget(index)">
				...
				<el-button title="删除" @click="handleWidgetDelete(index)" class="widget-action-delete" v-if="selectWg.key === item.key"
				  plain type="danger" size="mini">删除</el-button>
				<el-button title="复制" @click="handleWidgetClone(index)" class="widget-action-clone" v-if="selectWg.key === item.key"
				  plain type="primary" size="mini">复制</el-button>
			</div>
		</template>
	</draggable>
</template>
